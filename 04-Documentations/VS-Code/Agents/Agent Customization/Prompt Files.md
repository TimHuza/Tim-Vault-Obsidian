...
> [!Tip]
**Not sure which customization to use?** See the decision matrix to compare prompt files with custom agents, agent skills, and the other options.
...
> [!Tip]
In a monorepo, enable `chat.useCustomizationsInParentRepositories` to discover prompt files from the parent repository root. Learn more about parent repository discovery.
...
| `tools`         | No       | A list of tool or tool set names that are available for this prompt. Can include built-in tools, tool sets, MCP tools, or tools contributed by extensions. To include all tools of an MCP server, use the `<server name>/*` format.  <br>Learn more about tools in chat. |
...
Alternatively, use the **Chat: New Prompt File** or **Chat: New Untitled Prompt File** command from the Command Palette (`Ctrl+Shift+P`).
...
- In the Chat view, type `/` followed by the prompt name in the chat input field. [[Agent Skills|Agent skills]] also appear as slash commands alongside prompt files.
...
To sync your user prompt files, enable Settings Sync and run **Settings Sync: Configure** from the Command Palette (`Ctrl+Shift+P`). Select **Prompts and Instructions** from the list of settings to sync.
...
1. Select **Chat: Configure Prompt Files** from the Command Palette (`Ctrl+Shift+P`).
