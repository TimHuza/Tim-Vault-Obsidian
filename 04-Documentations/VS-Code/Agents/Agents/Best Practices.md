...
- **Exclude generated and noisy files from search.** Configure search.exclude and files.exclude so agent text search and grep stay focused on source code. See improve agent search with exclusion settings.

For full setup details, see Customize agent behavior in VS Codr.
...
| [[04-Documentations/VS-Code/Agents/Using Chat/Overview\|Ask (chat)]] | Questions, brainstorming, exploring ideas                        | "How does authentication work in this project?"             |
| [[Inline Chat\|Inline chat]]                                         | Targeted, in-place edits without switching context               | Refactoring a function, adding error handling               |
| [[04-Documentations/VS-Code/Get-Started/Overview\|Overview]]         | Multi-file changes that require autonomous planning and tool use | Implementing a feature end-to-end                           |
| [[Planning\|Plan]]                                                   | Structured planning before implementation                        | Designing an architecture or migration strategy             |
...
- **Offload well-defined tasks to background agents.** Use [[Copilot CLI|Copilot CLI]] or [[Cloud Agents|cloud agents]] when the task is clear enough that you don't need to watch every step.
    
- **Use cloud agents for team collaboration.** [[Cloud Agents|Cloud agents]] run remotely and create pull requests, making them ideal for tasks that benefit from team review or when you want to assign a GitHub issue directly to an agent.
...
- **Run parallel sessions for independent changes.** Break large tasks into independent subtasks and run them in parallel session, each focused on a different area of the codebase.
