...
![Screenshot of cloud agent session as a chat editor in VS Code.](https://code.visualstudio.com/assets/docs/agents/cloud-agents/cloud-agent-session.png)
...
VS Code supports different cloud agents, such as Copilot cloud agent and [[Third-Party Agents|third-party agents]] like Claude and Codex.

> [!Tip]
If you prefer an agent-first surface for running cloud agent sessions alongside Copilot CLI and Claude sessions across multiple projects, use the [[Agents Window|Agents window]]. Sessions are shared between the main VS Code window and the Agents window.
...
Learn more about third-party agents in VS Code and how to enable them.
...
If you prefer to work in the browser, you can also start cloud agent sessions directly from GitHub.com using the GitHub Copilot cloud agent.
...
1. In the Chat view, select **New Chat** from the session list dropdown and choose **Cloud** from the session type dropdown
    ...
    Alternatively, you can run the **Chat: New Cloud Agent** command from the Command Palette (`Ctrl+Shift+P`).
...
> [!Note]
If you have assigned an issue or pull request to the Copilot cloud agent on GitHub.com, the session automatically appears in the session list in VS Code.
...
2. Select the session type dropdown, and select **Cloud** to continue the session as a cloud agent.
    ...
    If you're using the [[Planning|Plan agent]], you can select **Continue in Cloud** from the **Start Implementation** dropdown to run the plan implementation in a cloud agent session
...
![Screenshot of cloud agent filter in VS Code Chat view.](https://code.visualstudio.com/assets/docs/agents/cloud-agents/cloud-agent-filter.png)
