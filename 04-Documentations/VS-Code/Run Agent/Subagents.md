...
> [!Tip]
To run deep research that produces a standalone, shareable report rather than feeding results back into your main conversation, use the built-in [[Copilot CLI#Run deep research with the research agent|research agent]] in Copilot CLI sessions.

For background on subagent concepts (context isolation, synchronous and parallel execution), see [[Agents#Subagents|Agents concepts]].
...
By default, subagents themselves cannot invoke further subagents. To enable recursive nesting, enable the `chat.subagents.allowInvocationsFromSubagents` setting. Learn more in [[Subagents#Nested subagents|Nested subagents]].
...
> [!Note]
Hover over a subagent section in the chat response to see the [[04-Documentations/VS-Code/Agents/Concepts/Language Models#AI credits and model costs|AI credits]] used by that subagent. This gives you more transparency into the cost of delegated work.
...
2. **Agent-configured model**: the [`model`](https://code.visualstudio.com/docs/agent-customization/custom-agents#_header-optional) property in the custom agent's `.agent.md` frontmatter. This can be a single model name or a prioritized list of models.
3. **Main model**: the model running the parent conversation.
...
To enable nested subagents, enable the c`hat.subagents.allowInvocationsFromSubagents` setting (`false` by default). When enabled, subagents can spawn their own subagents, up to a maximum nesting depth of 5.
